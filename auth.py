...
def epidemicDisease():
    ...
    res = ""
    if(output[0] == 1):
        res = "The given Disease have a high probability of becoming an Epidemic!"
    else:
        res = "The given Disease have a low probability of becoming an Epidemic!"
    ans = [{
        "dname" : dname,    
        "cases" : cases,
        "death" : death,
...
def covidDisease():
    ...
    rna = int(req.get("rna"))
    features = [gender,age,nocor,cardio,endo,tumor,resp,diges,renal,liver,fever,cough,chest,fatigue,diarrhea,rna]
    final = [np.array(features)]
